...
const {uniques2features, index2json} = require('./utils');
...
module.exports = (sources, tile, writeData, done) => {
    ...
    if (debug || process.env.REGEN) {
        ...
        // Note: using no indentations only reduce the filesize by ~15% (+1 readability vs. filesize)
        write.sync(debugPath + 'index.json', index2json(index));
    }
    if (verbose) {
    ...
    // Output Results
    write.sync(path.join(output, quadkey + '.json'), index2json(index));
    done(null);
};
